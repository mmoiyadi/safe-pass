...
**Independent Test**: Store 24 secrets across several types. Confirm that the full list is scannable without scrolling on a standard laptop screen, that selecting a row shows that secret's fields beside the list, and that reading, revealing and copying a field all behave exactly as they do today.
...
- **FR-014**: The secret list MUST show one line per secret: a title and a single summary line. Field values MUST NOT appear in list rows.
...
- **FR-027**: Every interactive target MUST be at least 44 pixels at narrow widths.
...
- **FR-030a**: The rail MUST NOT offer a "Recently used" scope. Nothing records when a secret was used, and last-changed answers a different question.
...
- **FR-031**: Fonts MUST be available without a network request, so that an offline unlock renders in the intended faces.
...
- **SC-001**: A vault of 24 secrets is fully scannable without scrolling on a 900-pixel-tall window; today the same vault requires several screens of scrolling.
...
- **SC-005**: Every user-facing string after the redesign is traceable to the codebase before it or to the handoff document.
